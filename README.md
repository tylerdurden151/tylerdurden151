<!-- Save this as README.md in a public repo named exactly: tylerdurden151/tylerdurden151 -->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0221,50:7b2ff7,100:00f5d4&height=220&section=header&text=TYLER%20DURDEN&fontSize=64&fontColor=ffffff&fontAlignY=38&desc=The%20first%20rule%20of%20my%20repo%3A%20always%20read%20the%20README.&descSize=18&descAlignY=60&animation=fadeIn" width="100%" alt="header" /> <a href="https://github.com/tylerdurden151"> 

<a href="https://github.com/tylerdurden151">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=800&color=00F5D4&center=true&vCenter=true&width=700&lines=%3E+Building+things+in+Python%2C+Java+%26+C%23;%3E+Simulating+satellites+and+shipping+dashboards;%3E+Full-stack+by+night%2C+debugger+by+necessity;%3E+The+first+rule+of+clean+code%3A+you+write+tests" alt="Typing SVG" />
</a>

![Profile Views](https://komarev.com/ghpvc/?username=tylerdurden151&style=for-the-badge&color=7b2ff7&label=VISITORS)
![Followers](https://img.shields.io/github/followers/tylerdurden151?style=for-the-badge&logo=github&color=00f5d4&labelColor=0d0221)
![Repos](https://img.shields.io/github/repositories/tylerdurden151?style=for-the-badge&logo=git&color=7b2ff7&labelColor=0d0221)

</div>

---

## `> whoami`

```python
class Developer:
    handle   = "tylerdurden151"
    focus    = ["Full-Stack Web", "Simulation", "Data Dashboards"]
    training = "Microsoft Software & Systems Academy (MSSA)"
    fuel     = ["coffee", "stack traces", "one more commit"]

    def status(self):
        return "Compiling. Please stand by."
```

---

## `> ls ./skills`

<div align="center">

![Python](https://img.shields.io/badge/Python-0d0221?style=for-the-badge&logo=python&logoColor=00f5d4)
![Java](https://img.shields.io/badge/Java-0d0221?style=for-the-badge&logo=openjdk&logoColor=7b2ff7)
![C#](https://img.shields.io/badge/C%23-0d0221?style=for-the-badge&logo=csharp&logoColor=00f5d4)
![.NET](https://img.shields.io/badge/.NET-0d0221?style=for-the-badge&logo=dotnet&logoColor=7b2ff7)
![Flask](https://img.shields.io/badge/Flask-0d0221?style=for-the-badge&logo=flask&logoColor=00f5d4)
![HTML5](https://img.shields.io/badge/HTML5-0d0221?style=for-the-badge&logo=html5&logoColor=7b2ff7)
![JavaScript](https://img.shields.io/badge/JavaScript-0d0221?style=for-the-badge&logo=javascript&logoColor=00f5d4)
![React](https://img.shields.io/badge/React-0d0221?style=for-the-badge&logo=react&logoColor=7b2ff7)
![Git](https://img.shields.io/badge/Git-0d0221?style=for-the-badge&logo=git&logoColor=7b2ff7)
![GitHub](https://img.shields.io/badge/GitHub-0d0221?style=for-the-badge&logo=github&logoColor=00f5d4)

</div>

---

## `> cat ./featured_projects`

| | Project | What it does | Stack |
|---|---|---|---|
| 🛰️ | [**LEOSatellite_Simulator**](https://github.com/tylerdurden151/LEOSatellite_Simulator) | Simulates orbital altitude loss from atmospheric drag on LEO satellites | `Java` |
| 📊 | [**academic-world-dashboard**](https://github.com/tylerdurden151/academic-world-dashboard) | Web dashboard that pulls from multiple database systems to explore the Academic World dataset | `Python` |
| 🔐 | [**HNCC_Full_Stack**](https://github.com/tylerdurden151/HNCC_Full_Stack) | Flask website with a working login system | `Python` `HTML` |
| 🎬 | [**Video_Link_Vault**](https://github.com/tylerdurden151/Video_Link_Vault) | Full-stack vault for saved video links with user authentication, categories, tags, platform detection and REST API integration | `React` `ASP.NET Core` `JavaScript` |
| 🧪 | [**MSSA_Mini_Project**](https://github.com/tylerdurden151/MSSA_Mini_Project) | Video Link Vault mini build: ASP.NET Core Web API + React with in-memory mock data and full CRUD for saved TikTok/YouTube/Instagram/Facebook links | `React` `ASP.NET Core` `JavaScript` |
| 🎓 | [**MSSA**](https://github.com/tylerdurden151/MSSA) | C# / .NET coursework, exercises, notes and projects | `C#` |

---

## `> git stats --all`

<div align="center">

<img src="https://streak-stats.demolab.com?user=tylerdurden151&theme=radical&hide_border=true&background=0d0221&ring=7b2ff7&fire=00f5d4&currStreakLabel=00f5d4" alt="Streak" />

<br/><br/>

<img src="https://ghchart.rshah.org/00f5d4/tylerdurden151" alt="Contribution graph" width="90%" />

</div>

---

## `> ./connect.sh`

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-tylerdurden151-0d0221?style=for-the-badge&logo=github&logoColor=00f5d4)](https://github.com/tylerdurden151)
<!-- Add your own links below, then delete this comment:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0d0221?style=for-the-badge&logo=linkedin&logoColor=7b2ff7)](https://linkedin.com/in/YOUR-HANDLE)
[![Email](https://img.shields.io/badge/Email-Say%20Hi-0d0221?style=for-the-badge&logo=gmail&logoColor=00f5d4)](mailto:YOUR-EMAIL)
-->

<sub>`while (alive) { learn(); build(); ship(); }`</sub>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00f5d4,50:7b2ff7,100:0d0221&height=120&section=footer" width="100%" alt="footer" />

</div>
